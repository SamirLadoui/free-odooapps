...
class TestReportToPrinter(TransactionCase):
    ...
    def setUp(self):
        ...
        self.server = self.env['sl.printing.server'].create({
            'name': 'Test CUPS', 'address': 'localhost', 'port': 631})
        self.printer = self.env['sl.printer'].create({
            'name': 'Office', 'system_name': 'office', 'server_id': self.server.id})
        self.other_printer = self.env['sl.printer'].create({
            'name': 'Warehouse', 'system_name': 'warehouse', 'server_id': self.server.id})
