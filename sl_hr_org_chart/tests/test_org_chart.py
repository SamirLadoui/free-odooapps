...
class TestOrgChart(HttpCase):
    ...
    def setUp(self):
        ...
        self.ceo = Employee.create({'name': 'Chief Exec', 'job_title': 'CEO'})
        self.head = Employee.create({
            'name': 'Head Of Sales', 'job_title': 'Director',
            'parent_id': self.ceo.id})
        self.rep_a = Employee.create({'name': 'Rep A', 'parent_id': self.head.id})
        self.rep_b = Employee.create({'name': 'Rep B', 'parent_id': self.head.id})
        self.loner = Employee.create({'name': 'No Manager'})
