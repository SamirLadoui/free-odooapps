...
class TestSchoolAttendance(TransactionCase):
    ...
    def setUp(self):
        ...
        self.standard = self.env['sl.school.standard'].create({
            'name': 'Grade 3', 'academic_year_id': self.year.id, 'capacity': 0})
        self.other_standard = self.env['sl.school.standard'].create({
            'name': 'Grade 4', 'academic_year_id': self.year.id, 'capacity': 0})
        ...
        self.applicant = self.env['sl.student'].create({
    ...
    def test_student_rate_counts_confirmed_registers_only(self):
        ...
        self.students[0].invalidate_cache(['attendance_rate'])
        self.assertEqual(self.students[0].attendance_rate, 0.0)
        self.students[1].invalidate_cache(['attendance_rate'])
