...
class TestSchoolFees(TransactionCase):
    ...
    def setUp(self):
        ...
        self.books = self._ensure_accounting()
        ...
        self.standard = self.env['sl.school.standard'].create({
            'name': 'Grade 5', 'academic_year_id': self.year.id, 'capacity': 0})
        self.other_standard = self.env['sl.school.standard'].create({
            'name': 'Grade 9', 'academic_year_id': self.year.id, 'capacity': 0})
        self.student = self.env['sl.student'].create({
            'name': 'Fee Payer', 'standard_id': self.standard.id})
        self.student.action_enrol()
        # Enrolment creates the contact; give it somewhere to post, since a
        # bare database has no chart of accounts to default from.
        self.student.partner_id.write({
            'property_account_receivable_id': self.books['receivable'].id,
            'property_account_payable_id': self.books['payable'].id,
        })

        self.structure = self.env['sl.fee.structure'].create({
            'name': 'Grade 5 Annual', 'academic_year_id': self.year.id,
            'standard_ids': [(6, 0, self.standard.ids)],
            'line_ids': [
                (0, 0, {'name': 'Tuition', 'amount': 1000.0}),
                (0, 0, {'name': 'Books', 'amount': 150.0}),
            ],
        })
        self.year_wide = self.env['sl.fee.structure'].create({
            'name': 'Whole Year Levy', 'academic_year_id': self.year.id,
    ...
    def test_student_totals(self):
        ...
        self.student.invalidate_cache(['fee_total', 'fee_outstanding'])
    ...
    def test_cancelled_fees_leave_the_totals(self):
        fee = self._fee()
        self.student.invalidate_cache(['fee_total'])
        ...
        def account_of(kind, name, code):
            if modern:
                ...
                found = accounts.search(
                    [('user_type_id', '=', self.env.ref(legacy_type[kind]).id)],
                    limit=1)
            if found:
                return found
            ...
            if modern:
                ...
                values['user_type_id'] = self.env.ref(legacy_type[kind]).id
            if kind != 'income':
