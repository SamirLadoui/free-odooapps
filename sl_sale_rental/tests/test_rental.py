...
class TestSaleRental(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.drill = cls.env['product.product'].create({
        ...
        cls.daily_only = cls.env['product.product'].create({
            'name': 'Daily Only', 'type': 'consu',
            'sl_rentable': True, 'sl_rental_price_day': 5.0})
        cls.not_rentable = cls.env['product.product'].create({
            'name': 'Sale Only', 'type': 'consu'})
        cls.start = date(2026, 6, 1)
