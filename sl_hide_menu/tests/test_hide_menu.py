...
class TestHideMenu(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.user = cls.env['res.users'].create({
            ...
            groups_field: [(6, 0, [cls.env.ref('base.group_user').id])],
        })
        ...
        cls.action_ref = '%s,%d' % (action._name, action.id)
        cls.parent = cls.env['ir.ui.menu'].create({'name': 'SL Parent Menu'})
        cls.child = cls.env['ir.ui.menu'].create({
            'name': 'SL Child Menu', 'parent_id': cls.parent.id})
        cls.grandchild = cls.env['ir.ui.menu'].create({
            'name': 'SL Grandchild Menu', 'parent_id': cls.child.id,
            'action': cls.action_ref})
        cls.unrelated = cls.env['ir.ui.menu'].create({
            'name': 'SL Unrelated Menu', 'action': cls.action_ref})
