...
class TestEmployeeShift(TransactionCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.employee = self.env['hr.employee'].create({'name': 'Shift Worker'})
        self.other = self.env['hr.employee'].create({'name': 'Second Worker'})
        self.day = self.env['sl.shift'].create({
            'name': 'Day', 'code': 'DAY', 'start_time': 9.0, 'end_time': 17.0})
        self.night = self.env['sl.shift'].create({
