...
class TestInvoiceFormat(TransactionCase):
    ...
    def setUp(self):
        ...
        self.partner = self.env['res.partner'].create({'name': 'Format Customer'})
        books = self._ensure_accounting()
        ...
        def account_of(kind, name, code):
            if modern:
                ...
                found = accounts.search(
                    [('user_type_id', '=', self.env.ref(legacy[kind]).id)],
                    limit=1)
            if found:
                return found
            ...
            if modern:
                ...
                values['user_type_id'] = self.env.ref(legacy[kind]).id
            if kind != 'income':
