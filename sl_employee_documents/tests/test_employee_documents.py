...
class TestEmployeeDocuments(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.today = fields.Date.context_today(cls.env['sl.employee.document'])
        cls.employee = cls.env['hr.employee'].create({'name': 'Doc Holder'})
        cls.passport = cls.env['sl.employee.document.type'].create({
            'name': 'Passport', 'expires': True, 'reminder_days': 30})
        cls.diploma = cls.env['sl.employee.document.type'].create({
