...
class TestHospital(TransactionCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.department = cls.env['sl.hospital.department'].create({
            'name': 'Cardiology', 'code': 'CARD'})
        cls.doctor = cls.env['sl.hospital.doctor'].create({
            'name': 'Ada Heart', 'department_id': cls.department.id,
            'consultation_fee': 50.0})
        cls.other_doctor = cls.env['sl.hospital.doctor'].create({
            'name': 'Grace Lung', 'department_id': cls.department.id})
        cls.patient = cls.env['sl.hospital.patient'].create({'name': 'John Smith'})
        cls.other_patient = cls.env['sl.hospital.patient'].create({'name': 'Jane Doe'})
        cls.slot = datetime(2026, 6, 1, 9, 0, 0)
