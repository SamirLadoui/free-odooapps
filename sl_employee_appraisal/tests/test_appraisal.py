...
class TestAppraisal(TransactionCase):
    ...
    def setUp(self):
        ...
        super().setUp()
        self.manager = self.env['hr.employee'].create({'name': 'The Manager'})
        self.employee = self.env['hr.employee'].create({
            'name': 'The Employee', 'parent_id': self.manager.id})
        self.category = self.env['sl.appraisal.category'].create({'name': 'Delivery'})
        self.light = self.env['sl.appraisal.criteria'].create({
            'name': 'Punctuality', 'category_id': self.category.id, 'weight': 1.0})
        self.heavy = self.env['sl.appraisal.criteria'].create({
            'name': 'Quality', 'category_id': self.category.id, 'weight': 3.0})
