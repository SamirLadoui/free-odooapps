...
class TestAccountStandardReport(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.account_a = cls._account(cls, 'SLT100', 'SL Report Test A')
        cls.account_b = cls._account(cls, 'SLT200', 'SL Report Test B')

        cls.partner_a = cls.env['res.partner'].create({'name': 'Ledger Partner A'})
        cls.partner_b = cls.env['res.partner'].create({'name': 'Ledger Partner B'})

        # One entry before the window, two inside it.
        cls._move(cls, date(2025, 12, 31), 100.0, cls.partner_a)
        cls._move(cls, date(2026, 2, 10), 250.0, cls.partner_a)
        cls._move(cls, date(2026, 3, 15), 75.0, cls.partner_b)
