...
class TestExam(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.standard = cls.env['sl.school.standard'].create({
            'name': 'Grade 6', 'academic_year_id': cls.year.id, 'capacity': 0})
        cls.other_standard = cls.env['sl.school.standard'].create({
            'name': 'Grade 7', 'academic_year_id': cls.year.id, 'capacity': 0})
        cls.subject = cls.env['sl.subject'].create({
    ...
    def test_student_average_counts_published_exams_only(self):
        ...
        self.students[0].invalidate_recordset(['exam_average'])
